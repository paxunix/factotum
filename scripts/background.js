...
Factotum.getSuggestion = function(command, argv)
{
    return {
        content: command + " " + argv.join(" "),
...
Factotum.onOmniboxInputChanged = function(text, suggestFunc)
{
    ...
    jQuery.each(commandList, function (i, el) {
        suggestions.push(Factotum.getSuggestion(el.names[0], argv));
    });
    ...
    else
    {
        ...
        var suggestion =
            Factotum.getSuggestion("'" + cmdName + "'",
